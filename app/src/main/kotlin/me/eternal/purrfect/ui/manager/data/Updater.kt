...
object Updater {
    ...
    private fun fetchLatestRelease(channel: Channel) = runCatching {
        val endpoint = Request.Builder().url("https://api.github.com/repos/particle-box/Purrfect/releases").build()
    ...
    private fun fetchLatestDebugCI() = runCatching {
        val actionRuns = OkHttpClient().newCall(Request.Builder().url("https://api.github.com/repos/particle-box/Purrfect/actions/runs?event=workflow_dispatch&branch=dev").build()).execute().use {
