...
class WhatsAppUiElementCaptureReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != Constants.WHATSAPP_UI_ELEMENT_CAPTURED_ACTION) return

        val rawValue = intent.getStringExtra(Constants.WHATSAPP_UI_ELEMENT_CAPTURED_VALUE_EXTRA).orEmpty()
        val isSelector = intent.getBooleanExtra(Constants.WHATSAPP_UI_ELEMENT_CAPTURED_IS_SELECTOR_EXTRA, false)
        ...
        runCatching {
            ...
            val uiElements = remoteContext.config.root.whatsapp.uiElements
            if (isSelector) {
