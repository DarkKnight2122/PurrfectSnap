...
# PurrfectSnap Telegram Notifier (Kal Edition)
# Custom Branding: Made by Kal with ❤️
...
# Determine Type & Icon
if [[ "$BRANCH_NAME" == "Kal" ]]; then
    TYPE_ICON="💎"
    TYPE_LABEL="ᴋᴀʟᴀᴅɪɴ ᴇᴅɪᴛɪᴏɴ"
...
# Search for APKs
SEARCH_PATHS=("app/build/outputs/apk")
FOUND_APKS=""

for path in "${SEARCH_PATHS[@]}"; do
    if [ -d "$path" ]; then
        FOUND=$(find "$path" -name "*.apk" -type f 2>/dev/null | grep -v "unsigned")
        if [ -n "$FOUND" ]; then
            FOUND_APKS+="$FOUND"$'\n'
        fi
    fi
...
⚠️ No APK assets discovered."
    ...
    while read -r apk; do
        ...
        MESSAGE="$MESSAGE
📦 <b>$ARCH</b> ($FILESIZE_MB MB)"

        # 1. Mirror: BashUpload
        BASH_URL=$(curl -s https://bashupload.com/ -T "$apk_target" | grep -o 'https://bashupload.com/[^ ]*' | head -n 1 | tr -d '\r\n')
        if [ -n "$BASH_URL" ]; then MESSAGE="$MESSAGE
 ├ <a href=\"$BASH_URL\">BashUpload</a>"; fi

        # 2. Mirror: Catbox
        CATBOX_URL=$(curl -s -F "reqtype=fileupload" -F "fileToUpload=@$apk_target" https://catbox.moe/user/api.php | tr -d '\r\n')
        if [[ "$CATBOX_URL" == "http"* ]]; then MESSAGE="$MESSAGE
 ├ <a href=\"$CATBOX_URL\">Catbox</a>"; fi

        # 3. Mirror: GoFile
        SERVER=$(curl -s https://api.gofile.io/servers | jq -r '.data.servers[0].name' 2>/dev/null)
        if [ "$SERVER" != "null" ] && [ -n "$SERVER" ]; then
            GOFILE_URL=$(curl -s -F "file=@$apk_target" "https://${SERVER}.gofile.io/uploadFile" | jq -r '.data.downloadPage' 2>/dev/null)
            if [ "$GOFILE_URL" != "null" ] && [ -n "$GOFILE_URL" ]; then
                MESSAGE="$MESSAGE
 └ <a href=\"$GOFILE_URL\">GoFile</a>"
            else
                MESSAGE="$MESSAGE
 └ (GoFile failed)"
            fi
...
MESSAGE="$MESSAGE
━━━━━━━━━━━━━━━━
🛠 <a href=\"https://github.com/$REPO/actions/runs/$RUN_ID\">View GitHub Action</a>"
