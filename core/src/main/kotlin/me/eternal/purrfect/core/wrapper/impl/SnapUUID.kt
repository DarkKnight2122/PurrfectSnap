...
class SnapUUID(
    ...
    private val uuidBytes by lazy {
        when {
            ...
            Purrfect.classCache.snapUUID.isInstance(obj) -> {
                obj?.getObjectField("mId") as ByteArray
            }
            Purrfect.classCache.snapShimsUUID?.isInstance(obj) == true -> {
                val any = obj as Any
    ...
    override var instance: Any?
        set(_) {}
        get() = Purrfect.classCache.snapUUID.getConstructor(ByteArray::class.java).newInstance(uuidBytes)
