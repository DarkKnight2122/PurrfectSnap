...
import me.eternal.purrfect.core.wrapper.impl.valdi.ValdiFunction
import me.eternal.purrfect.core.wrapper.impl.valdi.ValdiMarshaller
...
class ValdiHooks: Feature("ValdiHooks") {
    ...
    private fun newValdiFunction(block: ValdiMarshaller.() -> Boolean): Any? {
        val functionClass = Purrfect.classCache.valdiFunction ?: return null
    ...
    override fun init() {
        ...
        if (Purrfect.classCache.valdiFunction == null) {
        ...
        fun loadHooks() {
            ...
            context.native.setValdiLoader("""
                const i = setInterval(() => {
                    try {
                        const _runtimeName = "${if (Purrfect.classCache.nativeBridge.name == "com.snapchat.client.valdi.NativeBridge") "valdi" else "composer"}";
        ...
        Purrfect.classCache.nativeBridge.hook("registerNativeModuleFactory", HookStage.BEFORE) { param ->
