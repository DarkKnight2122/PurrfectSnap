...
import me.eternal.purrfectsnap.core.reddit.RedditRuntime
...
class XposedLoader : IXposedHookLoadPackage {
    override fun handleLoadPackage(param: XC_LoadPackage.LoadPackageParam) {
        ...
        XposedBridge.log(
            "Loading PurrfectSnap v${BuildConfig.VERSION_NAME}#${BuildConfig.GIT_HASH} into ${param.packageName} (package: ${BuildConfig.APPLICATION_ID})"
        )
        val initialized = AtomicBoolean(false)

        fun initFromContext(source: String, context: Context) {
            if (!initialized.compareAndSet(false, true)) {
                XposedBridge.log("PurrfectSnap already initialized for ${param.packageName}; ignored $source")
                return
            }
            runCatching {
                XposedBridge.log("PurrfectSnap $source for ${param.packageName}")
                RedditRuntime().init(context, param.classLoader)
            }.onFailure { throwable ->
                initialized.set(false)
                XposedBridge.log("PurrfectSnap failed during $source for ${param.packageName}: ${throwable.stackTraceToString()}")
            }
        }
        ...
        if (param.packageName == Constants.REDDIT_PACKAGE_NAME) {
            runCatching {
                ...
                XposedBridge.log("PurrfectSnap found Reddit application class: ${redditApplicationClass.name}")
                XposedBridge.hookAllMethods(
                ...
                XposedBridge.log("PurrfectSnap could not install Reddit application fallback hook: ${throwable.stackTraceToString()}")
            }
        }
